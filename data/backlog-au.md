...
- [x] ~~배너: images/uni/au-banner.jpg~~ **이 항목도 낡았다** — 로고 때와 같다.
      `au-banner.jpg`가 이미 있고 **35/35 페이지에 적용 중이다**(2026-08-03 확인).
      `build-uni.ps1`이 `images/uni/{id}.jpg`를 먼저 찾고 없으면 `{국가코드}-banner.jpg`로 넘어간다.
      학교 전용 배너를 넣고 싶으면 `au-{id}.jpg`를 추가하면 그 학교만 바뀐다.
...
- ~~기술이민(189/190/491)·생활비·학생비자 근로·장학금~~ → **2026-08-03 조사 완료. 글 1편 추가(총 26편).**
  근거는 `data/au-immigration-verified.md`, 글은 `skilled-migration`. 아래 절 참고
...
| 모나쉬 M.IT (c6001) | 55,500 | **여전히 "Fees to be advised"** → CRICOS로 대체 **57,200** | ✅ 사용자 결정 |
...
### 모나쉬 M.IT — CRICOS로 대체 (2026-08-03, 사용자 결정)

학교가 2026 금액을 안 내므로 **CRICOS 등록부 값을 쓰고 카드에 출처를 밝힌다**
(호주 학비에 CRICOS를 쓰는 기존 원칙 그대로, `$FEE_NOTE['AU']`와 같은 취급).

**Master of Information Technology `079055K`** — 104주 · 총 A$114,330 → **연 57,165 → 57,200**.

⚠ **그런데 옛 카드값 55,500의 출처가 밝혀졌다. 엉뚱한 행이었다.**
CRICOS에 `120133M | Non AQF Award | Information Technology Study Program | 52주 | 55,500`이 있다.
**학위 과정이 아니라 비학위 연수 프로그램**인데 금액이 딱 55,500이라 이게 잡혀 들어간 것으로 보인다.
→ **CRICOS를 쓸 때 `Course Level`을 반드시 확인할 것.** 이름이 비슷한 Non AQF Award·Diploma가
같은 학교에 섞여 있고, 모나쉬 IT만 해도 학위·복수학위·연수까지 25행이 나온다.

카드 `rank`에 "학비는 CRICOS 등록부 기준"을 적었고, 허브 모나쉬 PG 범위도 47,400~57,200으로 맞췄다.

### RMIT 의공학 — 2027 기준임을 카드에 명시 (2026-08-03, 사용자 결정)

RMIT는 2026 국제 학비를 더 이상 공시하지 않는다. 48,960은 **2027 입학자 기준**이다.
카드 `rank`를 "멜버른권 · 학비는 2027년 기준"으로 바꿔 연도를 드러냈다.
⚠ **연도를 `tuition` 필드에 넣으면 안 된다** — `krwSpan()`이 `\d{4,}`를 전부 금액으로 잡아
"2027"을 원화로 환산한다. 연도·출처 같은 부가 정보는 `rank`에 넣을 것.

### 플린더스 TESOL — 2025 금액으로 안내 (2026-08-03, 사용자 결정)

학교가 2026을 안 냈다. 확인된 2025 공시 **38,000**을 그대로 쓴다(옛 35,000은 CRICOS 환산).
`au-education-verified.md`에 ⚠2025 표시를 남겼으니 2026이 나오면 대조할 것.

## 🟢 Home Affairs 조사 완료 — 마지막 미반영 주제 묶음이 닫혔다 (2026-08-03)

근거 파일 신설: **`data/au-immigration-verified.md`**. 전부 정부 사이트 원문이다.
⚠ **immi.homeaffairs.gov.au는 WebFetch가 403이다. 실제 크롬으로 열 것.**

확인한 것: 학생비자 500(6년·AUD2,500·**2주 48시간**) · 재정 능력 요건 **29,710**(2024-05-10부터,
국가 최저임금의 75%) · 졸업생 비자 485 **3개 스트림**(학위 2~3년 / 디플로마 18개월 / 지방 2차 1~2년) ·
점수제 전체 표(합격선 65, 190 지명 5점, 491 지명 15점).

**글 1편 추가: `skilled-migration`** (총 26편, `INFO_COMMON`이라 전 전공에 노출).
축은 **"호주 학사만으로는 60점에서 멈춘다"** — 나이 30 + 영어 10 + 학사 15 + 호주 학업 요건 5.
모자란 5점을 채우는 셋(지방 학위 5 / Superior 영어 +10 / 491 지명 15) 중 **둘은 입학 전에 정해진다**는 것.

### 이번 조사에서 나온 주의사항

- ⚠ **29,710을 생활비로 옮기면 안 된다.** 심사 기준선이고, 호주 정부 유학 포털이 같은 페이지에
  *"actual living costs … may be much higher than what is needed for the visa"*라고 적어 두었다.
- ⚠ **Destination Australia 장학금은 끝났다.** 2024-25 예산으로 **2024년 7월 1일부터 신규 라운드 없음**
  (education.gov.au, 최종 수정 2025-04-01). 지방 진학의 근거로 이 장학금을 쓰면 안 된다.
  지방의 실익은 이제 **점수 5점 + 2차 485의 1~2년**이다.
- ⚠ **485는 스트림이 셋이고 신청 후 변경이 안 된다.** 디플로마 계열(Post-Vocational)은 18개월이고
  **자격이 기술직업목록의 직업과 연결돼야 한다** — 학위 스트림에는 없는 조건이다.
  디플로마 vs 학위 차이가 졸업 후에 두 번 나타난다는 뜻이라 `college-pathway` 글과 이어진다.

### 여전히 못 한 것

- **도시별 생활비 수치** — 정부 계산기가 숫자를 페이지에 안 싣는다(안내문에 2023-11 갱신이라 적힘).
  검증된 값이 없어 글에도 안 썼다. 다른 정부 출처를 찾으면 그때 넣을 것.
- **기술직업목록 ↔ 우리 31개 전공 매핑** — ANZSCO 코드 단위로 이어 붙여야 한다. 하면 전공별로
  "이 전공은 목록에 있다/없다"를 말할 수 있어 값이 크지만, 목록이 자주 바뀌어 유지 비용도 크다.
- **주별 491 지명 기준** — 주정부마다 다르고 수시로 바뀐다.
...
## ✅ 호주 글 원화 병기 — 완료 (2026-08-04)

**표 셀 111곳 전부 span 삽입 완료**(아래 "94곳"은 숫자 표기 정리 전의 옛 집계 — 셀 형태 표의
합계 111이 맞았다). 정규식 한 번에 111/111 잡혔고 가운뎃점 예외는 호주에 없었다.
빌드 후 4개 페이지에서 검증: 채움 ○ · 한↔영 4회 전환 유지 ○ · 375px 한 줄 ○ · 범위형(의대) ○.

⚠ **새 함정 — `data-en` 금액 셀은 span이 사라진다.** `<td data-en="From AUD N">` 셀
3곳(skilled-migration)이 영문 전환 시 innerHTML이 data-en으로 통째로 바뀌면서 span까지
지워졌다(영국 글엔 이 형태가 없어서 처음 나온 문제). **data-en 값 안에도 span을 넣어서**
해결했다 — 속성 안이라 span의 따옴표는 홑따옴표(`class='krw'`)로 쓴다. renderKrw()가
applyLang() 끝에서 다시 채우므로 그걸로 충분하다. **앞으로 금액이 든 data-en 셀을 만들면
span을 안팎 양쪽에 넣을 것.** 겸사겸사 `From AUD2,500`의 빠진 공백도 표기 규칙대로 고쳤다.

## ✅ 글 제목의 단일 출처 — 진단이 허브 색인을 읽는다 (2026-08-04)

### 무엇이 문제였나
**진단(`au-study-guide`)이 글 제목·설명을 복사해 두고 있었다.** 그래서 허브에서 제목을 고쳐도
진단은 옛 제목 그대로였다. 확인해 보니 **26편 전부 제목이, 25편은 설명까지 달랐다.**
남아 있던 게 하필 사용자가 없애라고 한 형태였다 — 제목 속 금액 7건("2년 학비가 5만 달러 차이"),
줄표 6건, 옛 숫자 표기. 게시판에는 새 제목, 진단에는 옛 제목이 나가고 있었다.

일단 스크립트로 맞췄다가(`sync-article-titles.ps1`), 같은 날 **구조를 바꿔 원인을 없앴다.**
그 스크립트는 이제 필요가 없어 지웠다.

### 지금 구조 — 제목은 `data/articles-{cc}.json` 한곳에만 있다
1. `build-uni.ps1`이 빌드할 때 **`data/articles-{cc}-index.json`** 을 같이 뽑는다.
   원본은 본문까지 있어 **236KB**라 통째로 주지 않는다. 결과 카드가 그리는 것만 담아 **9.5KB**다
   (`slug → { ko, en, cat, min, ver }`). 설명은 카드에 안 나오므로 넣지 않았다.
2. 진단은 이 색인을 fetch해서 제목을 채운다. **진단에 남은 것은 진단만 아는 두 가지뿐이다** —
   어느 전공에 붙일지(`INFO_ARTICLES` 키)와 `tracks`. 글 객체는 `{ slug, tracks }`로 줄었다.
3. 그래서 **글을 추가하려면 허브 JSON에 쓰고 진단엔 slug만 걸면 된다.** 제목은 안 옮겨 적는다.

- **허브와 진단은 같은 오리진**(`jaebong-choi.github.io`)이라 CORS가 필요 없다.
  로컬에서 시험할 때도 GitHub Pages가 `Access-Control-Allow-Origin: *`를 준다.
- 색인을 못 받으면 **글 섹션만 조용히 빠지고 결과의 나머지는 그대로 나온다.**
  색인에 없는 slug도 건너뛴다(빈 행을 만들지 않는다).
- 공유 링크로 바로 들어와 결과가 색인보다 먼저 그려질 수 있어서 **도착 시점에 한 번 더 그린다.**
  환율(`renderKrw`)과 같은 방식이다.
- ⚠ **`tracks`는 허브 값을 쓰지 않는다.** 허브 `tracks`는 게시판 필터용이고 진단은 결과 트랙용이라
  실제로 다른 게 있다(`aviation-cost`: 허브 bachelor, 진단 both). 색인에 `tracks`를 넣지 않은 이유다.
- ⚠ **진단 전에는 아무것도 그리지 않는 가드가 필요하다.** 없으면 색인이 도착하는 순간
  전공 무관 글 4편이 결과 화면도 없이 튀어나온다(만들다 실제로 겪었다).
- ⚠ **색인은 `Cache-Control: max-age=600`이다.** 제목을 고쳐도 이미 받아 간 브라우저에는
  최대 10분 뒤에 반영된다. 급하면 캐시를 비우고 볼 것.

검증: 콘솔 오류 0 · 색인 26편 수신 · 레거시 공유 링크 `#r=B-A-X-3-N-0` 결과 6편 정상 ·
한↔영 5회 전환 유지 · 학부/석사 × 3전공에서 트랙 필터가 이전과 동일 ·
색인을 끊었을 때 섹션만 숨고 학교 카드는 그대로.

### 이미 되어 있는 것 (건드릴 필요 없음)
- `js/guide.js`의 `renderKrw()` — 통화는 `<html data-country>`에서 고른다(au→AUD).
  `.fx-date` 문구는 진단 결과 페이지와 같다("원화는 2026. 8. 4. 환율로 환산한 참고 값입니다.")
- `css/guide.css`의 `.krw` — `display:block` + `white-space:nowrap`
- `scripts/build-uni.ps1`의 글 템플릿에 `<p class="fx-date"></p>` 들어 있음
- 캐시 버전 `guide.css?v=20260805` · `guide.js?v=20260805`

### 할 일 — `data/articles-au.json` 표 셀에 span 삽입
**대상 94곳.** `<span class="krw" data-min="60952" data-max="60952"></span>`를 금액 뒤, `</td>` 앞에 넣는다.

셀 형태는 다섯 가지다(수집 결과):
| 형태 | 개수 |
|---|---|
| `<td>AUD $N</td>` | 90 |
| `<td><strong>AUD $N</strong></td>` | 16 |
| `<td data-en="From AUD N">AUD N부터</td>` | 3 |
| `<td><strong>AUD N</strong></td>` | 1 |
| `<td>AUD $N~N</td>` | 1 |

정규식 하나로 다 잡힌다(영국 때 검증한 것을 AUD용으로 고친 것):
```
(<td(?:\s[^>]*)?>)((?:<strong>)?AUD\s*\$?([\d,]+)(?:\s*~\s*\$?([\d,]+))?[^<]{0,6}(?:</strong>)?)(</td>)
```
⚠ **JSON 안이라 따옴표는 `\"`로 써야 한다.** 그리고 이미 span이 있으면 건너뛸 것.
⚠ **ps1은 ASCII만.** 치환표가 한글이면 UTF-8 JSON으로 빼서 읽는다.

### ⚠⚠ 영국에서 겪은 함정 — 그대로 적용됨
`renderKrw()`는 **`applyLang()` 맨 끝에서** 불러야 한다. `applyLang()`이 `[data-en]` 요소의
`innerHTML`을 통째로 갈아 끼우기 때문에, 먼저 채우면 **언어를 바꿀 때마다 원화가 지워진다.**
(이미 그렇게 고쳐 뒀으니 확인만 하면 된다. 한↔영 4회 전환으로 시험할 것.)

⚠ **가운뎃점으로 두 금액을 이은 셀**(`£17,200 · £17,600` 같은 것)은 정규식에서 빠진다. 손으로 넣을 것.

### 검증
`serve-static.ps1`(4321) 띄우고 `http://localhost:4321/guide/au-info-*.html`에서
① 원화가 채워지는지 ② 한↔영 전환 후에도 남는지 ③ 375px에서 한 줄로 붙는지.
⚠ 프리뷰 창은 프로젝트 폴더 밖 자산을 못 불러오니 반드시 localhost로 볼 것.

---

### ⚠ 숫자 표기 규칙 (2026-08-04, 사용자 지시로 26편 전면 정리)

**본문·요약·제목의 숫자는 한글로 풀지 않고 숫자로 쓴다. 통화는 `AUD 60,952` 형식이다.**

| 위치 | 형식 | 예 |
|---|---|---|
| 본문·요약·제목 | **`AUD 60,952`** | "UQ는 IT 계열 대부분을 AUD 60,952 단일 밴드로 매깁니다" |
| **표 안** | **`AUD $60,952`** (기존 유지) | 사용자가 표는 그대로 두라고 지시 |
| 영문 `data-en` | **`AUD 60,952`** | "68,145 dollars" 같은 표현도 `AUD 68,145`로 |

바꾼 것: `6만 8천 1백 45달러` → `AUD 68,145` · `A$60,952` → `AUD 60,952` ·
`AUD $51,300` → `AUD 51,300` · `$60,900` → `AUD 60,900` · `7,080달러` → `AUD 7,080` ·
맨숫자 `32,500` → `AUD 32,500`.

⚠ **범위는 앞쪽에만 AUD를 붙인다.** `연 AUD 86,000~99,600` (뒤에 또 붙이지 않는다).
⚠ **`<strong>` 안에 숫자가 들어간 경우 치환이 빗나간다.** 태그째 잡아야 한다.

**작업 방식**: 표(`table-wrap`) 블록을 정규식으로 분리해 **표 밖에만** 치환을 적용했다.
스크립트는 ASCII로 두고 치환표는 별도 UTF-8 JSON으로 읽는다(ps1 한글 함정 회피).

**같이 잡은 값 오류 2건**: physio-ot의 UQ가 본문 `79,300`인데 표는 `79,370`이었다(표가 맞다).
vet-routes의 5년 총액도 본문 `448,500` 대 표 `448,560`이었다. 둘 다 표 기준으로 맞췄다.
...
- `culinary` → `au-commercial-kitchen.jpg` (상업용 주방, 캡션에 공립 AUD 23,000 · 브랜드 AUD 42,000)
- `nursing` → `au-nursing-simulation.jpg` (마네킹 술기 실습)

여전히 지키는 것: **캡션에 숫자를 넣어 사진을 근거의 일부로 만든다.** 웃는 학생 단체사진처럼
아무 데나 붙는 사진은 안 쓴다.

영국에서 먼저 도입한 원칙을 호주에도 적용했다. **26편 중 3편에만 넣었다.**

| 글 | 파일 | 캡션의 숫자 |
|---|---|---|
| `aviation-cost` | `au-flight-training.jpg` | 비행훈련비 $154,500 (학위 학비와 별도) |
| `it-cost` | `au-brisbane.jpg` | UQ $60,900 ↔ WSU $38,200 |
| `physio-ot` | `au-adelaide.jpg` | 플린더스 $55,100 (셋 중 최저) |

### ⚠ 호주는 대학 실물 사진이 스톡에 없다 — 도시로 간다
"University of Queensland" 검색은 Envato가 **close match 없음**을 띄우고 엉뚱한 대학 건물만 준다.
**아무 사암 건물에 "UQ $60,900"이라고 캡션을 달면 조작이다.** 그래서 대학 대신
**도시(브리즈번·애들레이드)**로 찍고, 캡션에도 도시 이름을 쓴 뒤 그 도시 학교의 금액을 붙였다.
사용자가 제안한 방식이고, 이러면 사진과 캡션이 둘 다 사실이다.

영국은 더럼 대성당·래드클리프 카메라·에든버러 스카이라인처럼 **한눈에 알아보는 장소**가 있어
대학 단위로 갈 수 있었다. 호주는 그게 안 되니 도시 단위가 상한이다.

`aviation-cost`만 예외로 대학이 아니라 **사물**(훈련용 경비행기)을 썼다. 글의 축이
"학위 학비에 비행훈련비가 안 들어간다"라 훈련기 사진이 그 숨은 비용을 그대로 보여준다.

**UQ가 `it-cost`·`physio-ot`·`vet-routes` 세 글에 나오지만 사진은 한 번만 썼다.**
같은 사진을 돌려 쓰면 그게 템플릿 티다.

### 글 제목 규칙 (2026-08-04, 사용자 지시로 27편 전면 교체)

**형식: `[전공명] 훅 한 문장.`**

```
[호주 공대] 순위가 아니라 인증으로 고릅니다.
[호주 간호] 학위가 있으면 2년, 없으면 3년.
[호주 항공 조종 과정] 학비가 전부가 아닙니다.
```

- **브래킷에 주제를 빼면 문장에서 주제를 반복할 필요가 없다.** 그래서 짧아진다.
  게시판에서 전공별로 훑기도 쉬워진다.
- ⚠ **줄표(—)로 주제와 설명을 잇지 말 것.** 옛 제목이 장황했던 주된 원인이다
  (`호주 공대 — 순위보다 Engineers Australia 인증을 먼저 보세요` 43자).
- **마침표는 붙인다**(사용자 결정). 27편 전부 통일돼 있으니 새 글도 맞출 것.
- 훅 원칙은 `/anthropic-skills:hook-generator`를 따른다:
  질문형 금지 · 숫자 우선 · 문장을 완결해 궁금증을 닫지 말 것.
- 영문 제목도 같은 브래킷 형식(`[Engineering] Choose on accreditation, not rankings`).

⚠ **H2 소제목은 손대지 않는다**(2026-08-04 사용자 결정). H2는 본문 흐름을 잡는 역할이라
전부 훅으로 바꾸면 산만해진다. 지금의 안내형 그대로 둔다.
