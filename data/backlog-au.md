...
## 🔴 진단의 글 제목이 허브와 따로 논다 — 26편 동기화 (2026-08-04)

**au-study-guide의 `INFO_ARTICLES`는 허브를 참조하지 않는다. 제목·설명을 복사해 둔다.**
그래서 허브에서 제목을 고쳐도 진단은 옛 제목 그대로다. 확인해 보니 **26편 전부 제목이 다르고
설명은 25편이 달랐다.** 남아 있던 게 하필 사용자가 없애라고 한 형태였다:
제목 속 금액 7건("2년 학비가 5만 달러 차이", "3년에 6만 8천 달러 차이"), 줄표 6건,
옛 숫자 표기("6만 8천 달러"). 게시판에는 새 제목, 진단에는 옛 제목이 나가고 있었다.

`scripts/sync-article-titles.ps1`로 맞췄다. **글 제목이나 설명을 고치면 이걸 다시 돌릴 것.**

```
powershell -File scripts/sync-article-titles.ps1 -HubJson data/articles-au.json \
  -DiagHtml ../au-study-guide/index.html            # 붙이면 -Apply
```
- 인자 없이 돌리면 **드라이런**이다. 글마다 `OK` / `EDIT 무엇이바뀜` / `WARN`을 찍는다.
- `url: HUB_ARTICLE + "<slug>.html"`로 글을 찾아 `ko`·`en`·`dko`·`den`을 허브 값으로 바꾸고
  `cat`·`min`·`ver`도 맞춘다(이번엔 이 셋은 이미 일치했다). **`tracks`는 손대지 않는다** —
  진단에서 어느 트랙에 띄울지는 진단 쪽 판단이라 허브의 게시판 필터값과 다를 수 있다.
- ⚠ **항목 형식이 두 가지다.** 첫 필드가 `{`와 같은 줄에 있는 것(`{ ko: "..."`)과
  줄이 나뉜 것이 섞여 있다. 줄 앞의 `{`까지 접두사로 잡아야 한다 — 처음에 16개를 놓쳤다.
- ⚠ **경로에 `클로드`가 들어가서 스크립트에 한글을 못 박는다.** 그래서 경로를 인자로 받는다.
  ps1 본문은 ASCII만이고 한글은 전부 파일에서 읽는다(`powershell-korean-pitfalls` 그대로).

검증: 진단을 4322 포트로 띄워 콘솔 오류 0, 런타임 항목 **47개(고유 글 26편)** 전부
`[전공명]` 형식 · 줄표 0 · 금액 표현 0. 레거시 공유 링크 `#r=B-A-X-3-N-0`으로 결과를 열어
글 카드 6장 정상, 영문 전환 시 한글 잔류 0.

⚠ **근본 원인은 그대로다.** 제목을 허브와 진단 두 곳에 복사해 두는 구조라 또 어긋난다.
진단이 허브의 `articles-{cc}.json`을 fetch하면 이 스크립트가 필요 없어진다(진단에 이미
fetch 호출이 하나 있다 — 환율). **구조를 바꿀지는 사용자 결정 사항으로 남겨 뒀다.**
