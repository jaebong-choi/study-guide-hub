...
## ✅ 진단 판정 로직 점검 (2026-08-06) — 간호를 디플로마 편입 분기에서 뺐다

au-study-guide `getPathwayLogic`이 간호+내신 4~6등급에 "Diploma Transfer"를 추천하고 있었는데,
허브 `pathway` 글은 **임상 실습 전공(간호·의대)은 디플로마 편입이 거의 없다**고 한다. 간호를 빼서
파운데이션으로 가게 했다. "편입이 보장됩니다"도 `college-pathway` 글(조건부 제안 — WAM+정원)에
맞춰 "성적 요건을 충족하면 편입합니다"로 바꿨다(ko·en, i18n `v=20260806`).
학교 카드의 "수능 XXX 또는 디플로마"는 **TAFE 간호 디플로마(입학 자격)** 라 그대로 뒀다.
MAJOR_TREE 31개 ↔ majorData 키 전수 대조 — 완전 일치.
