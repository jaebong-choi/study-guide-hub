...
### 다음에 이어갈 자리
- **빅토리아주(멜버른) 학교가 통째로 막혀 있다.** 모나쉬·멜버른대·RMIT·디킨·스윈번·라트로브
  전부 영어 요건 원문을 못 읽었다(403·JS·크롤러 차단). 그래서 새 카드가 NSW·QLD·SA에만 있고,
  진단에서 빅토리아를 고르면 "해당 지역 학교 없음" 안내가 뜬다. 뚫는 방법은
  `au-english-general.md`에 시도 기록과 함께 적어 뒀다. **학비는 CRICOS로 이미 뽑을 수 있으니
  영어 요건만 확인되면 카드를 바로 늘릴 수 있다.**
- **적산(QS)은 못 만들었다.** 학위명 자체가 본드대·QUT에만 있다. 건설관리로 대체했다.
- 새 전공 21종에는 아직 유학 정보 글이 붙어 있지 않다(`INFO_ARTICLES` 미등록).
  전공 무관 pathway 글만 학부 결과에 뜬다.
